...
module Make : functor (EXP : Typedjs_syntax.EXP) -> sig
  (** [from_exprjs] raises this exception. *)
  exception Not_well_formed of Pos.t * string

  val parse_annotation : Pos.t -> string -> Typedjs_writtyp.WritTyp.annotation

  (** @raise Not_well_formed if any of the well-formed criteria are
      violated (see above).
  *)
  val from_exprjs : 'a IdMap.t -> Exprjs_syntax.expr -> EXP.exp
end
