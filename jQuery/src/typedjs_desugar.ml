    ...
    **)
    let gen_bindings (dc : W.declComp) : typ IdMap.t =
      let generateSels ((classes, optClasses, ids) : W.attribs) (comb : Css.combinator) (sel : Css.t) (node : string) : Css.t =
        ...
        Css.concat_selectors sel comb (Css.singleton clsel) in
      let rec compileComp (ids : typ IdMap.t) (dc : W.declComp) (comb : Css.combinator) (sel : Css.t) : typ IdMap.t = 
        let (name, _, nodeType, attribs, content) = dc in
        let sels = generateSels attribs comb sel nodeType in
        let new_ids = 
          IdMap.add 
            name
        ...
        List.fold_left (fun ids dcc -> 
          match dcc with
          | W.DNested dc -> compileComp ids dc Kid sels
