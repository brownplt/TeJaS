...
module MakeExt
  ...
  (TypeScriptKinding : TYPESCRIPT_KINDING
  ...
  (Desugar : TypeScript_desugar.TYPESCRIPT_DESUGAR
   with type typ = TypeScript.typ
...
struct
  ...
  open Env
  open TypeScriptKinding

  module Strobe = TypeScript.Strobe
  ...
  let bind' x b (env : env) = bind x (TypeScript.embed_b b) env
  let bind_id x t (env : env) = bind' x (Strobe.BTermTyp (TypeScript.extract_t t)) env
  let bind_lbl x t env = bind' x (Strobe.BLabelTyp (TypeScript.extract_t t)) env
  ...
  let bind_rec_typ_id (x : id) recIds (t : typ) (env : env) =
    ...
    raw_bind_typ_id x (TypeScript.extract_t t) (TypeScript.extract_k k) env
  ...
  let bind_recursive_types (xts : (id * typ) list) (env : env) =
    let env' = List.fold_left (fun ids (x, t) ->
      raw_bind_typ_id x (TypeScript.extract_t t) Strobe.KStar ids) env xts in
  ...
  let unchecked_bind_typ_ids (xts : (id * typ) list) (env : env) =
    List.fold_left (fun ids (x, t) -> raw_bind_typ_id x (TypeScript.extract_t t) Strobe.KStar ids) env xts
    ...
    | Strobe.TRef (n, Strobe.TObject o), Strobe.KStar ->
      ...
      let add_field env (x, pres, t) =
        if pres = Strobe.Present then
          ...
          | Some s -> bind_id s (TypeScript.embed_t (Strobe.TRef (n, t))) env
          | None ->
  ...
  let extend_global_env env lst =
    ...
    let desugar_typ p t = TypeScript.extract_t (Desugar.desugar_typ p t) in
    let rec add recIds env decl = match decl with
      | EnvBind (p, x, typ) ->
        (try
         ...
         with Not_found ->
           let t = TypeScript.embed_t (Strobe.expose_twith env (desugar_typ p typ)) in
      ...
      | EnvType (p, x, writ_typ) ->
        (try
         ...
         with Not_found ->
           ...
           let t'' = TypeScript.expose_twith env t' in
           let k = kind_check env recIds t'' in
           bind_typdef x (extract_t (apply_name (Some x) t'')) (extract_k k) env)
      | EnvPrim (p, s) ->
        TypeScriptKinding.new_prim_typ s;
        env
      ...
      | Decl (_, (name,_,_,_,_)) ->
        failwith (sprintf "TYPESCRIPT_extend_global_env: impossible: all local structure declarations should have already been compiled, but Decl %s was not." name)
