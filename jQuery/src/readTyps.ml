...
open Typedjs_parser
open Typedjs_lexer
module JS = JavaScript_syntax

module H = Hashtbl
module W = Typedjs_writtyp.WritTyp

type ann = (Prelude.Pos.t * W.annotation) list

type comments = (Prelude.Pos.t * string) list

type typ_db = (Prelude.Pos.t, W.annotation) H.t
...
let new_decls (comments : (Pos.t * string) list) : W.env_decl list =
  List.concat (ListExt.filter_map (fun c -> if is_typedecl c then Some (parse_typedecl c) else None) comments)
    

let annotations_of_comments (comments : (Pos.t * string) list) : (Pos.t * W.annotation) list = 
...
let assoc_annotations (js_ast : JS.prog) ann_lst =
  ...
  | JS.Prog (_, stmts) -> match fold_left stmt ann_lst stmts with
    | [] -> tbl
    | _ -> failwith "extra annotations"

let read_typs (js_ast : JS.prog) (comments : (Pos.t * string) list) : typ_db =
