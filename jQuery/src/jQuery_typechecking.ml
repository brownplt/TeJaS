...
let rec check (env : env) (senv : structureEnv ) (default_typ : TypImpl.typ option) (exp : exp) (typ : TypImpl.typ) : unit =
  try trace "Check" (fun exp -> check' env senv default_typ exp typ) exp
...
and check' (env : env) (senv : structureEnv) (default_typ : TypImpl.typ option) (exp : exp) (typ : TypImpl.typ) : unit =
  (* Printf.eprintf "Check': checking %s : %s\n" (string_of_exp exp) (string_of_typ typ); *)
  match exp with
  | EFunc(p, name, args, body) ->
    begin match typ with
    | TInter(_, t1, t2) ->
      (try (with_typ_exns (fun () -> check env senv default_typ exp t1))
       with Typ_error (p, e) ->
         typ_mismatch p
      ...
      (try (with_typ_exns (fun () -> check env senv default_typ exp t2))
       with Typ_error (p, e) ->
         typ_mismatch p
    ...
    | TUnion(_, t1, t2) ->
      (try (with_typ_exns (fun () -> check env senv default_typ exp t1))
       with Typ_error _ -> check env senv default_typ exp t2)
    | TArrow(_, arg_typs, None, ret) ->
      if not (List.length arg_typs = List.length args) then
        ...
        check env senv default_typ body ret
    | TArrow (_, arg_typs, Some vararg_typ, result_typ) ->
      ...
      check env senv default_typ body result_typ
    | t ->
  ...
  | _ ->
    ...
    let synth_typ = synth env senv default_typ exp in
    (* Printf.printf "Checking %s <?: %s\n" (string_of_typ synth_typ) (string_of_typ (expose_simpl_typ env typ)); *)
    if not (subtype_typ ((* lax *)true) env senv synth_typ typ) then begin
...
and synth (env : env) (senv : structureEnv) (default_typ : typ option) (exp : exp) : typ =
  trace "Synth" (synth' env senv default_typ) exp
and synth' env senv default_typ exp : typ =
  ...
  | EAssertTyp(p, t, e) ->
    ...
    let _ = check env senv default_typ e t in
    t
  | EId (p, x) -> begin
    try
      lookup_id x env
  ...
  | EApp(p, f, xs) ->
    let rec check_app tfun =
      ...
      | TArrow(_, args, None, ret) ->
        ...
        begin try List.iter2 (check env senv default_typ) xs args
      ...
      | TArrow (_, args, Some var, ret) ->
        if (List.length args > List.length args) then
          ...
          List.iter2 (check env senv default_typ) xs args
        else begin
          ...
          List.iter2 (check env senv default_typ) req_xs args;
          List.iter (fun t -> check env senv default_typ t var) var_xs
        end;
        ret
      | TInter(_, t1, t2) ->
    ...
    check_app (synth env senv default_typ f)
  | ECheat(p, t, _) -> t
  | EFunc _ -> TArrow(None, [], Some TBot, TTop) (* TODO *)
  | ELet (_, x, e1, e2) -> synth (bind_id x (synth env senv default_typ e1) env) senv default_typ e2
  | ESeq (_, e1, e2) -> begin match synth env senv default_typ e1 with
      TBot -> (* e1 will not return; no need to typecheck e2 *)
        TBot
      | _ -> synth env senv default_typ e2
  end
  | ERec (p, binds, body) ->
    ...
    let tc_bind (x, t, e) = check env senv default_typ e t in
    List.iter tc_bind binds;
    synth env senv default_typ body
